...
@ExtendsDirective
class Product(
    @property:ExternalDirective val upc: Int,
    val review: List<Review>? = null
) {
    fun review(): List<Review>? {
        return ArrayList(
            reviews.filter {
                it.value.product.upc == upc
            }.values)
    }
}
...
val productResolver = object : FederatedTypeResolver<Product> {
    override suspend fun resolve(representations: List<Map<String, Any>>): List<Product?> = representations.map {
        ...
        val upc = it["upc"]?.toString()?.toIntOrNull() ?: throw InvalidProductUpcException()
